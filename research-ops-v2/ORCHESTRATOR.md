# Research Orchestrator — 세션 실행 지시서 v2 (프로젝트 무관)

> **이 문서는 오케스트레이션을 맡은 에이전트가 읽고 그대로 실행하는 지시서다.**
> v1의 6-STEP 구조·역할 경계·불변 규칙을 **그대로 유지**하고, v2의 정형화(라벨 상태기계·GATE/EVIDENCE 스키마·CI 재검증·자동 backfill)를 **증분으로** 얹는다.
...
### STEP 0 — self-check (신설, 실패 시 즉시 중단·보고)
...
for f in PROTOCOL.md templates/issue_dev.md templates/issue_gate.md; do
  [ -f "research-ops-v2/$f" ] || { echo "STOP: 누락 $f"; exit 1; }
done

# 4) CODE_REPO 접근
gh repo view "$CODE_REPO" >/dev/null || { echo "STOP: CODE_REPO 접근 불가 — $CODE_REPO"; exit 1; }
```
통과하면 STEP 1로. (워크플로 2개·NTFY_TOPIC secret·대시보드는 README §2 적용 순서에서 1회 셋업 — self-check는 존재만 권고 확인.)

### STEP 1 — 입력 수집 (v1 유지)
- §1 입력 확인. 누락 필수·ENV는 **한 번에 모아서** 묻는다.
- PLAN이 초안이면 STEP 2, 정형화 최종본이면 STEP 3으로.

### STEP 2 — 계획서 보강·정형화·HTML (초안일 때만, v1 유지)
...
### STEP 3 — MGMT milestone 이슈 + GitHub 네이티브 milestone (증분)
v1의 milestone 이슈 생성에 **GitHub 네이티브 milestone**을 병행한다 (라벨과 함께 기계 상태를 이룬다).
...
### STEP 4 — P{k} 명세 + dev 이슈 자동 생성 (증분: 자동 backfill)
...
- **`scripts/setup_phase.sh P{k} <P{k}.md 경로>`** 실행 → `@goal:` 블록 파싱 → dev 이슈 생성(`[P{k}-M{n}]`, 라벨 `state:ready,type:dev,phase:P{k}`, HUMAN GATE 포함 시 `type:gate`, milestone 연결) → **`## Goal ↔ Issue Map` 표 자동 생성·커밋**. (v1의 수동 backfill 제거)
- 여기서 **너는 멈춘다.** 실제 실행은 gjc.

### STEP 5 — gjc 위임 → 완료 확인 → 다음 단계 (v1 유지 + 라벨/검증)
...
gjc ralplan --interactive "P{k}.md 명세를 읽고 실행 계획 수립. research-ops-v2/PROTOCOL.md 를 준수하라(라벨 전이·GATE/EVIDENCE/PROGRESS 스키마)."
...
별도 상태 파일을 만들지 않는다. **진실의 원천은 GitHub이다** (v1 §5 계승):
...
research-ops-v2/ORCHESTRATOR.md 와 PROTOCOL.md, templates/ 를 읽어라. STEP 0 self-check부터 진행한다.
...
규칙: 코드 구현은 하지 마라(gjc 몫). 단계별로 STEP 4까지 하고 gjc 실행을 나에게 넘겨라.
gjc 킥오프에는 PROTOCOL.md 준수를 명시하라. 내가 "P{k} 완료"라고 하면 완료 검증(§4) 후 다음 단계로.
모호하면 스스로 정하지 말고 물어라. GitHub 쓰기가 필요하면 그 시점에 토큰/gh 인증을 확인하라.
```

이어가는 프롬프트: **"P{k} 완료. 상태 확인하고 P{k+1} 진행해줘."** → STEP 4→3 루프.
...
- 산출물 뼈대: `templates/phase_spec.md`(v1 계승)·`templates/issue_dev.md`(v2)·`templates/issue_gate.md`(v2)
- 자동화: `scripts/bootstrap_project.sh`·`scripts/setup_phase.sh`·`scripts/status.sh`
- 이벤트: `workflows/gate-notify.yml`·`workflows/evidence-verify.yml` (CODE_REPO)
- 조회: `dashboard/index.html` (MGMT_REPO, GitHub Pages)
