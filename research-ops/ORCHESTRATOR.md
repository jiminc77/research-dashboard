...
```text
필수:
  PLAN         연구 계획서 (경로 또는 URL) — 단계(P0..Pn)와 각 단계 범위가 담긴 문서
...
선택(기본값 있음):
  ...
  ENV          실행 환경 4종 (명세에 박제됨):
                 ...
                 — 없으면 물어본다. 이건 P{k}.md의 "실행 환경(고정)" 블록에 그대로 들어간다.
토큰:
...
## 2. 세션 프로세스 (사용자가 원하는 5단계)

### STEP 1 — 입력 수집·계획서 파싱
- PLAN을 읽어 **단계 목록(P0..Pn)과 각 단계의 목표·게이트·산출물**을 추출한다.
- 누락된 필수 입력·ENV가 있으면 사람에게 **한 번에 모아서** 묻는다.
- 결과를 요약 제시하고 STEP 2 진행 승인을 받는다 (자동 폭주 금지).

### STEP 2 — 관리 레포에 Milestone 자동 생성 (전체 단계)
- `templates/issue_milestone.md` 형식으로 P0..Pn milestone 이슈 본문을 만든다.
- `templates/bootstrap_project.sh`로 MGMT_REPO에 milestone 이슈 n+1개 생성.
- 대시보드 문서(README의 Current Snapshot·Milestone 표, `docs/implementation/…_plan.md`)를 이 프로젝트로 초기화.
- P0을 **Current**, 나머지 **Backlog**로.

### STEP 3 — P0 단계 명세 + 구현 레포 이슈 자동 생성
- `templates/phase_spec.md` 뼈대로 **`P0.md`** 작성: 계획서의 P0 섹션 + ENV(고정) + 전역 규칙 + `@goal` 마일스톤(M0..Mj) + HUMAN GATE 표시 + 기계 검증 가능한 Exit.
- `P0.md`를 CODE_REPO에 push (README·.gitignore·STEP_LOG 골격 포함).
- `templates/setup_phase.sh`로 CODE_REPO에 dev 이슈 M0..Mj 생성 → **생성된 번호를 `P0.md`의 Goal↔Issue 매핑표에 반영**하고 재push.
- 여기서 **너는 멈춘다.** 실제 실행은 gjc가 한다 (§3의 실행 명령을 사람에게 안내).

### STEP 4 — gjc 실행 위임 → 완료 확인 → 다음 단계
- 사람은 CODE_REPO에서 `gjc ralplan → ultragoal`로 그 단계를 구현한다 (아래 §3).
- 사람이 **"P{k} 완료"** 라고 하면 너는 **단계 완료 검증(§4)** 을 수행한다.
- 검증 통과 시: 대시보드 갱신(해당 milestone → Done, 필요 시 Decision 이슈 기록, 승계 리스크·확정 수치를 다음 단계 입력으로 이월) → **P{k+1}에 대해 STEP 3을 반복.**
- 검증 미통과 시: 무엇이 빠졌는지 보고하고 그 단계에 머문다 (다음 단계 선행 금지).
...
- 잘 채운 실제 예시: **DGCC** (`DGCC/P0.md`, dashboard P0–P7 이슈, `docs/reports/P0_pilot_gates.md`)
