...
- **회고의 산출물로 이전 단계 리포트 HTML**을 `templates/phase_report_guide.md` 형식으로 생성해 MGMT_REPO `docs/reports/P{k}_report.html` 에 커밋한다 (이미 존재하면 갱신). 대시보드 "단계 리포트" 카드가 이 파일을 자동으로 링크한다.
