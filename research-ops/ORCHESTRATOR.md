...
선택(기본값 있음):
  ...
  ENV/실행설정  세션 B가 gjc를 띄울 원격 환경 (projects/{slug}/project.yml에 박제):
...
**먼저 관리 레포 `projects/{slug}/project.yml`을 읽어** CODE_REPO / SSH_HOST / WORKDIR / 모델(MODEL_MAIN·MODEL_EXEC) / ntfy 토픽명을 파악한다. 이 값들이 이번 세션과 세션 B 지시서·단계 전환의 진실이다.

- **파일이 있으면**(기존 프로젝트): 값을 로드하고 STEP 1로. 셋업 불필요.
- **파일이 없으면**(신규 프로젝트): 셋업 지시서(웹폼)가 준 값으로 진행하고, **`projects/{slug}/project.yml`을 생성·커밋하는 것이 STEP 0의 산출물**이다. 아래 셋업 모드를 수행한다.

#### STEP 0 셋업 모드 (신규 프로젝트일 때만)
1. **GitHub 인증** 확인 — 세션 A는 `gh` 없는 LLM 세션이면 사람이 준 fine-grained PAT(두 레포 Contents RW + Issues RW)로 REST API를 쓴다. (사람이 로컬 셸에서 직접 부트스트랩할 때만 `gh auth login` 후 `gh auth status`.)
2. **라벨 18종**(양 레포) + **phase 마일스톤** 생성 — PROTOCOL §1의 state 6종·보조 라벨(type:*, phase:*, proj:*) 세트. `templates/bootstrap_project.sh` 사용.
3. **워크플로 4종(pr-verify·phase-transition·gate-notify·evidence-verify) 커밋** — 관리 레포 `research-ops/workflows/`의 `pr-verify.yml`·`gate-notify.yml`·`evidence-verify.yml`·`phase-transition.yml`을 CODE 레포 `.github/workflows/`에 복사·커밋.
4. **NTFY_TOPIC secret은 사람 몫** — 값을 네가 만들지 말고, `gh secret set NTFY_TOPIC -R {CODE_REPO}` 명령과 넣을 값(추측 불가 문자열)을 **보여주고 사람의 확인을 받는다.**
5. **`projects/{slug}/project.yml` 커밋** — `projects/_template.yml`을 채워 관리 레포 `projects/`에 커밋 (code_repo·ssh_host·workdir·모델·ntfy 토픽명·phases).
...
### STEP 1 — 입력 수집
- PLAN(초안 또는 최종본)·CODE_REPO·설정 등 §1 입력을 확인한다 (STEP 0에서 로드한 `projects/{slug}/project.yml` 우선).
...
- "프로젝트 설정?" → MGMT_REPO `projects/{slug}/project.yml` (code_repo·ssh_host·workdir·모델·ntfy).
