...
- CODE 레포 `docs/reports/`(또는 `outputs/reports/`)의 이전 단계 리포트.
...
- **회고의 산출물로 이전 단계 리포트 HTML**을 `templates/phase_report_guide.md` 형식으로 생성해 MGMT_REPO `docs/reports/P{k}_report.html` 에 커밋한다 (이미 존재하면 갱신). 대시보드 "단계 리포트" 카드가 이 파일을 자동으로 링크한다. 커밋 전 `research-ops/scripts/lint_report.sh` 통과 필수 — 섹션 id·어투 계약·self-contained 요건을 기계 검증한다.
...
- **마지막 단계 리포트 생성**: 마지막 단계 P{n}에 대한 회고를 촉발하는 킥오프가 없으므로, 여기서 `templates/phase_report_guide.md` 형식으로 `docs/reports/P{n}_report.html` 을 직접 생성·커밋한다 (이미 있으면 갱신).
- (선택) **전체 단계 리포트 색인**: `docs/reports/`의 P0..Pn 리포트를 한눈에 링크하는 색인 페이지를 만든다. 대시보드 "단계 리포트" 카드가 개별 리포트를 자동 링크하므로 필수는 아니다.
...
- "무엇이 확정됐나?" → `docs/reports/`의 게이트/결정 리포트, Decision 이슈, `### GATE VERDICT`.
