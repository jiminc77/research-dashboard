...
| 파일 | 역할 | 대상 |
|---|---|---|
| `ORCHESTRATOR.md` | **세션 실행 지시서.** (계획서 초안 + 구현 레포) → 계획서 보강·정형화·HTML → milestone 생성 → 단계별 명세·이슈 → gjc 위임 → 완료 확인 → 다음 단계 반복. **여기서 시작.** | 에이전트 |
| `WORKFLOW.md` | 운영 계약서 — 3층위 모델·레포 구성·단계 생애주기·규칙 | 에이전트 + 사람 |
| `NEW_PROJECT_PROMPT.md` | 착수/단계전환/재개 프롬프트 템플릿 (복붙용) | 사람 |
| `templates/plan_refinement.md` | 초안 계획서 보강·정형화 절차 (문헌조사·gap·적대적 리뷰·정형화) | 에이전트 |
| `templates/research_plan_template.md` | 정형화 최종 계획서의 구조 뼈대 | 에이전트 |
| `templates/plan_html_guide.md` | 계획서 → 단일 HTML 생성 가이드 (설계 스펙 적용) | 에이전트 |
